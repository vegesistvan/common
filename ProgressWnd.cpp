...
LPCTSTR szSection = L"Settings";    
LPCTSTR szEntryX  = L"X"; 
LPCTSTR szEntryY  = L"Y"; 
...
BOOL CProgressWnd::Create(CWnd* pParent, LPCTSTR pszTitle, BOOL bSmooth /* = FALSE */)
{
    ...
    bSuccess = m_Text.Create(L"", WS_CHILD|WS_VISIBLE|SS_NOPREFIX|SS_LEFTNOWORDWRAP,TempRect,this,IDC_TEXT);
...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////
BOOL CProgressWnd::GoModal(LPCTSTR pszTitle /*=L"Progress")"*/, BOOL bSmooth /*=FALSE*/)
{
    CWnd *pMainWnd = AfxGetMainWnd();

    if (!::IsWindow(m_hWnd) && !Create(pMainWnd, pszTitle, bSmooth))return FALSE;
...
void CProgressWnd::SetWindowSize(int nNumTextLines, int nWindowWidth /*=390*/)
{
    ...
    if (pDC)
	{
        ...
        CString str = L"M"; 
        for (int i = 0; i < nNumTextLines-1; i++) str += L"\nM"; 
        pDC->DrawText(str, TextRect, DT_CALCRECT|DT_NOCLIP|DT_NOPREFIX);
        TextRect.right	= TextRect.left + nWindowWidth;
        CancelSize		= pDC->GetTextExtent(m_strCancelLabel + L"  " ) + CSize(EdgeSize.cx*4, EdgeSize.cy*3 );
...
int CProgressWnd::StepIt()                
{
	...
	if((m_nPrevPos+m_nStep)>m_nMaxValue)return m_nPrevPos;		// ezt én tettem bele!!
...
int CProgressWnd::SetPos(int nPos)                    
{
	...
	if(nPos>m_nMaxValue)return nPos;		// ezt én tettem bele!!
    ...
    if (nPercentage != m_nPrevPercent) 
    {
        ...
        strTitle.Format(L"%s [%d%%]",m_strTitle,nPercentage);
...
void CProgressWnd::SetText(LPCTSTR fmt, ...)
{
    ...
    va_start(args, fmt);
    _vstprintf_s(buffer,sizeof(buffer), fmt, args);
    va_end(args);

    m_Text.SetWindowText( buffer );			// ez nem akar mûködni, midig lerobban!!
}
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////    
...
{
    if (m_bPersistantPosition)SaveCurrentSettings();

    m_bCancelled = TRUE;
    Hide();

    if(m_bModal)PostMessage(WM_CLOSE);

    CWnd *pWnd = AfxGetMainWnd();
    if(pWnd && ::IsWindow(pWnd->m_hWnd))pWnd->SetForegroundWindow();
}
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////    
BOOL CProgressWnd::DestroyWindow() 
{
    ...
    if (m_bModal)
    {
        m_bModal		= FALSE;
        CWnd *pMainWnd	= AfxGetMainWnd();
        if (m_wRenenableWnd)m_wRenenableWnd->EnableWindow(TRUE);
    }
